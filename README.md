...
- RSS/Atom 与无 RSS 官网页面自动抓取、关键词过滤、去重和规则评分
...
编辑 `config/sources.json`。每个信源包含名称、地址、信源等级和语言；`type: "html"` 可直接采集厂商官网页面。抓取失败不会清空旧数据，现有内容会继续保留，页面会显示信源健康度。

## 每日更新闭环

- GitHub 仓库使用 `refresh-news.yml`，每天 07:30（Asia/Shanghai）抓取并提交新数据，提交后自动触发 Pages 发布。
- 当前 Codex 项目已创建同一时间运行的本地自动任务：刷新数据、检查信源质量、构建测试并把新版本发布到现有私有站点。
- 日报摘要、趋势温度、匹配数量和信源健康度均由本次抓取结果重新计算，不再使用固定展示值。
